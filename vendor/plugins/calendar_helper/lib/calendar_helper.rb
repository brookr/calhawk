...
module CalendarHelper
  ...
  def calendar(options = {}, &block)
    ...
    raise(ArgumentError, "No month or week given") unless options.has_key?(:month) || options.has_key?(:week)
    ...
    if options.has_key?(:week)
      #commercial dates count by week and day number, day 1 = monday, day 0 = error.
      cday = Date.commercial(options[:year], options[:week], 1) - 1 + options[:first_day_of_week]
      options[:month] = (cday+7).month
      first = cday
      last = first + 41
    else
    ...
    xm.table("class" => options[:table_class], "border" => 0)
    ...
    beginning_of_week(first, first_weekday).upto(first - 1) do |d|
      cal << %(<td class="#{options[:other_month_class]} #{options[:prev_month_class]})
    ...
    (last + 1).upto(beginning_of_week(last + 7, first_weekday) - 1)  do |d|
      cal << %(<td class="#{options[:other_month_class]} #{options[:next_month_class]})
      cal << " weekendDay" if weekend?(d)
      if options[:accessible]
